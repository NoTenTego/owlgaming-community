...
function loadOneFaction(row)
	...
	if theTeam then
		...
		exports.anticheat:setEld(theTeam, "note", row.note == exports["mysql"]:mysql_null() and "" or row.note, 'one')
		exports.anticheat:setEld(theTeam, "fnote", row.fnote == exports["mysql"]:mysql_null() and "" or row.fnote, 'one')
		exports.anticheat:setEld(theTeam, "phone", row.phone ~= exports["mysql"]:mysql_null() and row.phone or nil, 'one')
			...
			if (factionID) then
				...
				if result then
					...
					for _, row in ipairs(result) do
						...
						if phone and row.faction_phone ~= exports["mysql"]:mysql_null() and tonumber(row.faction_phone) then
	...
	if hasMemberPermissionTo(client, factionID, "manage_finance") then
		...
		if query then
			while true do
				...
				if reason == exports["mysql"]:mysql_null() then
					reason = ""
				end

				local from, to = "-", "-"
				if row["characterfrom"] ~= exports["mysql"]:mysql_null() then
				...
				if row["characterto"] ~= exports["mysql"]:mysql_null() then
