...
function loadOneGate(gateID)
	...
	if row then
		...
		if tempObject then
			...
			if row["triggerDistance"] == exports["mysql"]:mysql_null() then triggerDistance = false else triggerDistance = tonumber(row["triggerDistance"]) or 35 end
			if row["triggerDistanceVehicle"] == exports["mysql"]:mysql_null() then triggerDistanceVehicle = false else triggerDistanceVehicle = tonumber(row["triggerDistanceVehicle"]) or 35 end
			if row["sound"] == exports["mysql"]:mysql_null() then gateSound = false else gateSound = tostring(row["sound"]) end
