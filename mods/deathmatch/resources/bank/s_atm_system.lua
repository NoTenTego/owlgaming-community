...
function applyForNewATMCard(replacingOldATMCard, limitType)
	...
	while true do
		...
		if string.len(numSum) == 19 then
			...
			if (not checkNumber or checkNumber["card_number"]==exports["mysql"]:mysql_null() or checkNumber["card_number"] ~= numSum) then
				break
			end
		end
	end
...
function withdrawATMMoneyPersonal(amount, theATM)
	...
	local balance = nil
	if not balanceCheck or balanceCheck["bankmoney"] == exports["mysql"]:mysql_null() then
