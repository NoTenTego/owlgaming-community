	...
	elseif type == "username" then
		...
		while q do
			...
			for k, v in pairs( result ) do
				if v == exports["mysql"]:mysql_null() then
					result[k] = nil
				else
			...
			if result1 and #result1 > 0 then
				...
				if result then
					...
					while true do
						...
						if result1["lastlogin"] == exports["mysql"]:mysql_null() then
...
local function showAlts(thePlayer, id, creation)
	...
	if result then
		...
		while true do
			...
			if row['lastlogin'] ~= exports["mysql"]:mysql_null() then
				text = text .. " - " .. tostring( row['lastlogin'] )
			end
			
			if creation and row['creationdate'] ~= exports["mysql"]:mysql_null() then
