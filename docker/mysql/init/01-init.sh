...
mysql -u root -p"${MYSQL_ROOT_PASSWORD}" <<-EOSQL
    ...
    -- Użytkownik bazy MTA (mysql_native_password wymagane przez mta_mysql.so)
    CREATE USER IF NOT EXISTS '${MTA_DATABASE_USERNAME}'@'%'
        IDENTIFIED WITH mysql_native_password BY '${MTA_DATABASE_PASSWORD}';
    ...
    CREATE USER IF NOT EXISTS '${CORE_DATABASE_USERNAME}'@'%'
        IDENTIFIED WITH mysql_native_password BY '${CORE_DATABASE_PASSWORD}';
